...
The repository links modeled relationships to cited research and records scope limitations alongside those links. [STATUS.md](STATUS.md) summarizes the current scientific boundary, while machine-readable definitions and evidence remain versioned with the repository; see the [variables registry](model/variables.json), [mechanism links](model/links.json) and [evidence snapshot](model/evidence_snapshot.json). The retained evidence set is an auditable project resource, not a systematic review or a dataset for population calibration.
...
For readers who want to inspect or run the computational model, CBD requires **Python 3.12 or newer**. To inspect the current default-branch state, clone the repository and run:
...
- **Model definitions and evidence records:** [variables registry](model/variables.json), [mechanism links](model/links.json) and [evidence snapshot](model/evidence_snapshot.json)
- **Executable implementation:** [model core](src/cognitive_epistemic_model/model.py) and [simulation engine](src/cognitive_epistemic_model/simulation.py)
- **Reproducibility and verification:** [CI reproducibility contract](tests/test_ci_reproducibility_contract.py) and [registry schema](schemas/variable.schema.json)
- **Change history:** [CHANGELOG.md](CHANGELOG.md)
- **Development continuity:** [DEVELOPMENT.md](DEVELOPMENT.md)
- **Released snapshots:** [GitHub Releases](https://github.com/LaurentiuStaicu/cognitive-belief-dynamics/releases) and the corresponding Git tag
