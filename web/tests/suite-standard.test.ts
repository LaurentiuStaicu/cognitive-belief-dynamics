...
test('Product Concept Recovery is questions-first, not a full-map landing page',()=>{
 ...
 assert.match(shellSource,/data-product-view="atlas"/);
...
test('Usefulness Gate 3: internal codes are confined to Research / Technical provenance',()=>{
 const split=shellSource.split('RESEARCH / TECHNICAL PROVENANCE');
 assert(split.length>=2);
 const publicTemplate=split[0];
 ...
 assert.match(shellSource,/Research \/ Provenance|Research \/ Proveniență/);
 assert.match(shellSource,/technical-node-register/);
 assert.match(understandingSource,/scrubTheoryTechnicalIdentifiers/);
 assert.match(understandingSource,/technical file — see Research \/ Provenance/);
});
...
test('Usefulness Gate 5-6: mechanism evidence cards expose synthesis quality and effect sizes only conditionally',()=>{
 ...
 for(const evidence of mechanismEvidence){
  ...
  for(const field of [evidence.strength.en,evidence.studyType.en,evidence.studies.en,evidence.populations.en,evidence.uncertainty.en,evidence.heterogeneity.en,evidence.moderators.en,evidence.quality.en,evidence.replication.en,evidence.competing.en,evidence.limitations.en])assert(field.length>15,evidence.nodeId);
  assert(evidence.sources.length>0,evidence.nodeId);
 }
 const familiarity=mechanismEvidence.find(item=>item.nodeId==='familiarity')!;
 assert.match(familiarity.studies.en,/182 studies/);
 assert.match(familiarity.populations.en,/31,184/);
 assert.match(familiarity.effectSize!.en,/g=0\.37/);
 assert.match(familiarity.quality.en,/risk-of-bias|Risk-of-bias/i);
 assert.match(shellSource,/WHAT EVIDENCE SUPPORTS IT/);
 assert.match(shellSource,/HOW LARGE IS THE EFFECT/);
 assert.match(shellSource,/evidence\.effectSize\?/);
});
...
test('Usefulness Gate 7: Intervention Explorer covers the nine empirical classes without becoming a recommender',()=>{
 ...
 for(const item of interventionEvidence){
  assert(item.targetNodes.every(id=>nodeIds.has(id)),item.id);
  for(const field of [item.targetMechanism.en,item.studyType.en,item.population.en,item.outcome.en,item.effect.en,item.uncertainty.en,item.heterogeneity.en,item.duration.en,item.moderators.en,item.adverse.en,item.directness.en])assert(field.length>15,item.id);
  assert(item.sources.every(source=>source.url.startsWith('https://')),item.id);
 }
 assert.match(shellSource,/INTERVENTION EVIDENCE EXPLORER/);
 assert.match(shellSource,/Intervention Evidence ≠ “CEM recommends this action”/);
});

test('Usefulness Gate 8: Theory/Learn is a broad manual and technical identifiers are not its public language',()=>{
 assert(theoryTopics.length>=20);
 ...
 assert.match(shellSource,/The complete CEM manual|Manualul complet al CEM/);
 assert.match(shellSource,/data-suite-theory-chapter/);
 assert.match(understandingSource,/public reader explains the theory in natural language/);
});

test('Full Model Atlas is secondary, filtered and progressive rather than a spaghetti graph',()=>{
 assert(atlasGroups.length>=9);
 for(const group of atlasGroups)assert(group.nodes.every(id=>nodeIds.has(id)),group.id);
 assert.match(shellSource,/FULL MODEL ATLAS/);
 assert.match(shellSource,/atlasFilter/);
 assert.match(shellSource,/atlasZoom/);
 assert.match(shellSource,/relations appear only around the selected mechanism/);
 assert.match(shellSource,/relatedEdges=selected\?/);
});
...
test('responsive product layout gives the pathway the dominant surface and the mechanism context is dismissible',()=>{
 assert.match(shellCss,/\.pathway-workspace\{width:min\(100%,calc\(100vw - 2\.4rem\)\);min-height:72vh/);
 assert.match(shellSource,/data-close-mechanism/);
 assert.match(shellCss,/\.context-drawer\{position:fixed/);
 assert.match(shellCss,/@media\(max-width:900px\)[\s\S]*\.context-drawer\{position:static/);
});
...
test('frozen boundaries remain frozen: no MOD.15 implementation, Advanced mode, calibration upload or Flatpak work',()=>{
 ...
 assert.equal(mainSource.includes('id="releaseVersion"'),false);
});
