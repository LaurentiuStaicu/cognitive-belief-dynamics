...
test('OA-6C canonical context links only to existing Semantic Spine entities and a real Theory chapter',()=>{
 const ids=new Set(variables.map(item=>item.id));
 for(const id of uncertaintyCanonicalContext.semanticIds)assert(ids.has(id),id);
 assert(theory.some(item=>item.source_paths.en.endsWith('/12-interventions.md')));
 assert.equal(uncertaintyCanonicalContext.theorySlug,'interventions');
});

test('OA-6C registry remains explicitly non-probabilistic for all initial uncertainty objects',()=>{
 ...
 assert(registry.uncertainties.every(item=>item.probability_status==='NOT_AVAILABLE'));
 assert.equal(registry.uncertainties.filter(item=>item.quantification_status==='FINITE_SCENARIOS').length,2);
});
