...
import {graphReading} from './graph-reading';
import type {ExplanationData} from './explanation';
import {dependencies} from './dependencies';
import {scenarioShortName,tourName} from './labels';
...
export function mountVisualStage(cy:Core,lang:'ro'|'en',runs:Run[],explanations:ExplanationData,initial:string,time:number,show:(id:string)=>void,edgeDetail:(id:string)=>void,open:(id:string,step:number)=>void,onFrame:(id:string,step:number)=>void){
 ...
 const reading=()=>{if(!cy.nodes(':selected').length){q('stageReading').innerHTML='';return;}const e=explanations.runs.find(r=>r.id===run.id)!.frames[index];q('stageReading').innerHTML=graphReading(selected,run.frames[index],run.frames[index-1],e,lang);};
 const update=()=>{
 ...
 const inspect=()=>{document.getElementById('stageNeighbours')?.remove();const node=cy.nodes(':selected').first();if(!node.length){focus();return;}selected=node.id();reading();const panel=document.createElement('div');panel.id='stageNeighbours';
 panel.innerHTML=`<h3>${tr('Vecinătatea vizibilă a factorului','Visible factor neighborhood')}: ${selected}</h3>`+(['target','source'] as const).map((end,i)=>`<p><strong>${i?tr('Ieșiri','Outputs'):tr('Intrări','Inputs')}</strong></p><div class="dependency-buttons">${dependencies.filter(e=>e[end]===selected&&cy.getElementById(e.id).length).map(e=>`<button data-stage-node="${i?e.target:e.source}">${i?e.target:e.source}</button><button data-stage-edge="${e.id}">${e.source} ${polarity(e.id)} → ${e.target}</button>`).join('')||tr('Niciuna în această vedere.','None in this view.')}</div>`).join('');
 ...
 // Existing selection handlers render synchronously; append the accessible neighborhood afterwards.
 cy.on('tap','node',()=>{endTour();inspect();});cy.on('tap','edge',()=>{endTour();q('stageReading').innerHTML='';focus();});
 document.getElementById('variable')!.addEventListener('change',()=>{endTour();inspect();});document.getElementById('dependency')!.addEventListener('change',()=>{endTour();q('stageReading').innerHTML='';focus();});
 document.getElementById('detail')!.addEventListener('click',()=>{endTour();if(cy.nodes(':selected').length)inspect();else q('stageReading').innerHTML='';focus();});
 q<HTMLInputElement>('stageFocus').onchange=focus;
 q<HTMLSelectElement>('stageRun').onchange=()=>{endTour();run=runs.find(r=>r.id===q<HTMLSelectElement>('stageRun').value)!;index=0;q<HTMLInputElement>('stageTime').value='0';q<HTMLInputElement>('stageTime').max=String(run.frames.length-1);q<HTMLButtonElement>('stageNext').disabled=true;update();};
 ...
 const tour=()=>{const id=q<HTMLSelectElement>('stageTour').value;const route=tours[id].filter(key=>cy.getElementById(key).length);if(!route.length)return;q<HTMLInputElement>('stageFocus').checked=true;show(route[tourStep]);inspect();q('stageTourText').textContent=`${tr('Traseu explicativ','Explanatory route')}: ${route.join(' → ')} · ${tourStep+1}/${route.length}. ${tr('Factor selectat','Selected factor')}: ${route[tourStep]}.`;q<HTMLButtonElement>('stageNext').disabled=tourStep>=route.length-1;q<HTMLButtonElement>('stageBack').disabled=tourStep===0;};
 q('stageStart').onclick=()=>{activeTour=true;tourStep=0;run=runs.find(r=>r.id===q<HTMLSelectElement>('stageTour').value)!;index=run.id==='repetition'?4:run.id==='source'?8:5;q<HTMLSelectElement>('stageRun').value=run.id;q<HTMLInputElement>('stageTime').value=String(index);update();tour();};
 const endTour=()=>{activeTour=false;q<HTMLButtonElement>('stageNext').disabled=true;q<HTMLButtonElement>('stageBack').disabled=true;q('stageTourText').textContent=tr('Selectează un tur pentru parcurgere ghidată. Explicația urmărește factorul și pasul selectat.','Select a tour for guided exploration. The explanation follows the selected factor and time step.');};
 q('stageNext').onclick=()=>{if(activeTour){tourStep++;tour();}};q('stageBack').onclick=()=>{if(activeTour){tourStep--;tour();}};q<HTMLSelectElement>('stageTour').onchange=endTour;
 update();inspect();
}
