...
function shell() {
 ...
 if(view==='learning') mountUnderstanding(document.getElementById('content')!,lang,runs,m1Editorial,m1Presentation,m1Targets,{chapters:theoryIndex,glossary:theoryGlossary,variables,modules,references,validations:validationTests,releaseTag:release.release_tag},target=>{stop();if(location.hash.startsWith('#understanding/')&&!target.startsWith('learning'))history.pushState({cemReturnToUnderstanding:true},'',location.href);const [next,id,time]=target.split(':');view=next;if(id){selected=id;step=time===undefined?0:Number(time);}shell();document.getElementById('content')!.scrollIntoView({block:'start'});});
...
addEventListener('popstate',refreshUnderstandingRoute);
