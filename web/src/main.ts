...
import {dependencies,extraNodes,hydrateComputationalDependencies,type ComputationalDependencyRegistry} from './dependencies';
let graphMode='core';
let graphFocus='all';
...
async function init(){const dependencyData=load<ComputationalDependencyRegistry>('computational_dependencies');[variables,links,modules,references,oddProcesses,subsystems,m1Targets,m1Editorial,m1Presentation,m1Access,theoryIndex,theoryGlossary,validationTests]=await Promise.all([load<Variable[]>('variables'),load<Link[]>('links'),load<Module[]>('modules'),load<Reference[]>('references'),load<OddProcess[]>('processes'),load<Subsystem[]>('subsystems'),load<EmpiricalTarget[]>('empirical_targets'),load<M1EditorialData>('m1_editorial'),load<M1PresentationData>('m1_presentation'),load<M1AccessData>('m1_access'),load<TheoryChapter[]>('theory_index'),load<TheoryGlossaryEntry[]>('theory_glossary'),load<TheoryValidation[]>('validation_tests')]);hydrateComputationalDependencies(await dependencyData);runs=(await load<{runs:Run[]}>('runs')).runs;planning=await load<PlanningData>('interventions');explanations=await load<ExplanationData>('explanations');shell();}
