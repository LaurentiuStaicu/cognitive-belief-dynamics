...
type Link = {id:string; source:string; target:string; relation_type:string; polarity:string; mechanism_evidence_status:string; functional_form_status:string};
...
function updateFrame(){
 ...
 const line=(key:keyof Frame,color:string)=>`<polyline fill="none" stroke="${color}" stroke-width="3" points="${run().frames.slice(0,step+1).map(f=>`${px(f.time)},${py(Number(f[key]))}`).join(' ')}"/><circle cx="${px(step)}" cy="${py(Number(f[key]))}" r="5" fill="${color}"/>`;
 document.getElementById('chart')!.innerHTML=`<svg viewBox="0 0 748 290" role="img" aria-label="${tr('Evoluția convingerii, distribuirii și corecției; valorile exacte sunt în tabelul de mai jos.','Belief, sharing and correction trajectories; exact values are in the table below.')}"><title>${runName(selected)}</title>${[0,.25,.5,.75,1].map(v=>`<line x1="50" x2="710" y1="${py(v)}" y2="${py(v)}" stroke="#29364a"/><text x="36" y="${py(v)+5}" text-anchor="end">${v}</text>`).join('')}${run().frames.map(f=>`<text x="${px(f.time)}" y="270" text-anchor="middle">${f.time}</text>`).join('')}<line x1="${px(step)}" x2="${px(step)}" y1="24" y2="242" stroke="#78869a" stroke-dasharray="4 5"/>${line('belief','#5ce0ce')}${line('share_probability','#b5a0ff')}${line('correction','#ffcc80')}</svg>`;
...
function renderStructure(){
 ...
 graph=cytoscape({container:document.getElementById('cy'),elements:[...variables.map(v=>({data:{id:v.id,label:v.short_name,full:v.label[lang]},position:positions[v.short_name]})),...links.map(l=>({data:{id:l.id,source:l.source,target:l.target,label:l.relation_type==='MODERATING'?tr('moderează','moderates'):tr('influențează','influences')}}))],style:[{selector:'node',style:{'label':'data(label)','width':64,'height':64,'background-color':'#213c50','border-width':2,'border-color':'#5ce0ce','color':'#ecf4fc','text-valign':'center','font-size':20}},{selector:'edge',style:{'curve-style':'bezier','target-arrow-shape':'triangle','line-color':'#7890a9','target-arrow-color':'#7890a9','label':'data(label)','color':'#c3d0e0','font-size':12,'text-margin-y':-12,'width':2}},{selector:':selected',style:{'border-color':'#ffcc80','line-color':'#ffcc80','target-arrow-color':'#ffcc80'}}],layout:{name:'preset',padding:50},minZoom:0.4,maxZoom:2,wheelSensitivity:0.2});
...
function linkDetail(l:Link){return `<p class="eyebrow">${tr('RELAȚIE ÎN REGISTRU','REGISTERED LINK')}</p><h3>${text(variables.find(v=>v.id===l.source)!.label[lang])} → ${text(variables.find(v=>v.id===l.target)!.label[lang])}</h3><dl class="link-data"><dt>${tr('Tip','Type')}</dt><dd>${text(l.relation_type)}</dd><dt>${tr('Polaritate','Polarity')}</dt><dd>${text(l.polarity)}</dd><dt>${tr('Etichetă de evidență declarată','Declared evidence label')}</dt><dd>${text(l.mechanism_evidence_status)}</dd><dt>${tr('Formă funcțională','Functional form')}</dt><dd>${text(l.functional_form_status)}</dd></dl><p class="note">${tr('Registrul inițial nu include încă referințe bibliografice pentru fiecare relație. Eticheta de evidență nu este o verificare independentă a acesteia.','The initial registry does not yet include per-link bibliographic references. Its evidence label is not an independent verification.')}</p>`;}
...
async function init(){[variables,links,modules]=await Promise.all([load<Variable[]>('variables'),load<Link[]>('links'),load<Module[]>('modules')]);runs=(await load<{runs:Run[]}>('runs')).runs;shell();}
