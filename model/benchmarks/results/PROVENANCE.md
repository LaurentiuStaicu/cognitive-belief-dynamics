...
## Checksum-file semantics

The retained `.sha256` files are historical integrity records, but they do **not** all hash the neighboring retained result file. Their exact target semantics are machine-readable in `checksum_semantics.json`.

- Phase F/G: hash the original authoritative aggregate JSON generated by the historical workflow; those JSON bytes are not retained in the current checkout, while the CSV result surface is retained.
- Phase I: the file is a 12-entry shard manifest over historical `screen-*/result.json` files; it is not the checksum of the retained screening CSV.
- Phase K: hashes the retained authoritative JSON itself and is recomputed by CI.
- Phase M: hashes the historical Actions source aggregate `phase-m-summary.json`; the retained authoritative JSON is a normalized repository representation that carries the same source digest in its provenance fields.

A checksum manifest is therefore interpreted together with provenance and target identity, never merely by similarity between the checksum filename and a neighboring result filename.
