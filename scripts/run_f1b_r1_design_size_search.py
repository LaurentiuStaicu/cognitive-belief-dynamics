...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the non-authoritative F1b R1 participant×item design-size recovery search."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("model/benchmarks/f1b_r1_design_size_search.json"),
    )
    ...
    result = run_r1_design_size_search(config)
    result["provenance"] = {
